...
export class PurchaseRfqModel extends BaseModel<PurchaseRfqData> {
    useMocks: boolean = false;

    constructor() {
        super('PurchaseRfqStore', 'purchase-rfqs');
    }

    setFields(data: Partial<PurchaseRfqData>): void {
        this.store.fields = { ...PurchaseRfqModel.fields, ...data };
    }
        ...
        for(let i = 0; i < AllModelData.length; i++) {
            ...
            responseCardData.value.push({
                ...
                body: [{
                    ...
                    'links': [
                        {
                            link: '/purchase/rfqs/' + AllModelData[i].id,
                ...
                filterable: {
                    ...
                    filters: {
                        status: {
                            ...
                            callback: (selected) => {
                                let selectedValue = selected == 'Yes' ? true : false;
                                return AllModelData[i].isExpired == selectedValue;
                            }
                        },
                        quotes: {
                            ...
                            callback: (selected) => {
                                if(selected == 'Yes'){
                                    return AllModelData[i].quoted_enquires_count > 0;
                                } else {
                                    return AllModelData[i].quoted_enquires_count == 0;
                                }
                            }
