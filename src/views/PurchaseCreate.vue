      ...
      <div class="container">
        <ion-header>
          <ion-buttons slot="start">
            <ion-back-button text="" default-href="/purchase">
    ...
    <ion-content class="spaero-page">
      <div class="container container--narrow">
        ...
        <form class="spaero-form">
          ...
          <AddAircraft type="basic" />
          <Checkbox label="Aircraft on ground" v-model="create_rfq_form.aircraft_on_ground" style="margin-top: 15px;"/>
...
const purchaseRfqModel = new PurchaseRfqModel();
...
const handleAircraftsUpdated = async () => {
  await updatePageContent();
};

onMounted(() => {
  updatePageContent();
  EventBus.on('aircraftAdded', handleAircraftsUpdated);
});
...
const savePurchaseRfq = async () => {
  try {
    addPartNumber();
    create_rfq_form.value.part_number = create_rfq_form.value.part_numbers.join(',');
    const rfq = await purchaseRfqModel.create(create_rfq_form.value);
    router.push(`/purchase/rfqs/${rfq.id}?isNew=true`);
  } catch (error) {
